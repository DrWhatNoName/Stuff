...
class PB
{
	...
	{
		return false; //TODO: fix OTP. Use EOTP.
		...
		global $PLAUSIBLE_DENIABILITY;
		if($PLAUSIBLE_DENIABILITY && mt_rand() % 5 == 0)
		{
	...
	public static function AddUpdatePassword($id, $key, $token, $description, $passuser, $password, $url, $encryptionsalt)
	{
		$token = security::sqlsani($token);
	...
	public static function Logout($user = "")
	{
		...
		if(!empty($user))
		{
			$rand = security::sqlsani(security::get256() . security::get256() . security::get256());
			self::SetOTPKey($user,$rand);
			self::RemoveOTPKey($user);
		}
	...
	{
		global $CHANGE_COOKIE_KEYS;
		if($CHANGE_COOKIE_KEYS && !empty($username))
		{
	...
	private static function AddPlausibleDeniability()
	{
		//PLAUSIBLE DENIABILITY: add random passwords
		...
		if($PLAUSIBLE_DENIABILITY && mt_rand() % 5 == 0) 
		{
			self::AddUpdatePassword("", bin2hex(security::SuperRand(3)), bin2hex(security::SuperRand()), "fakedescription", "fakeuser", "fakepassword", "fakeurl", bin2hex(security::SuperRand()));
		}

		global $PLAUSIBLE_DENIABILITY;
		if($PLAUSIBLE_DENIABILITY && mt_rand() % 5 == 0) //add a random notepad witha 1/10 probability
		{
			$length = mt_rand() % 20 + 2;
			$data = "";
			for($i = 0; $i < $length; $i++)
	...
	public static function GetOTPKey($user)
	{
		...
		//Grab the id from the user row 
		$result = mysql_query("SELECT otp FROM accounts WHERE (username='$user')");
		$ary = mysql_fetch_array($result);
		$otp_id = $ary['otp'];
		$otp_id = security::sqlsani($otp_id);

		//Grab the real key from the ram table
		$otp_ram = mysql_query("SELECT data FROM ramtable WHERE(id='$otp_id')");
		$otp_ram = mysql_fetch_array($otp_ram);
		$otp = $otp_ram['data'];
		global $ENCRYPT_COOKIE_KEY;
		if($ENCRYPT_COOKIE_KEY)
			$otp = security::ServerDecrypt($otp);
		return $otp;
	}
	
	public static function RemoveOTPKey($user)
	{
		$user = security::sqlsani($user);
		...
		mysql_query("DELETE FROM ramtable WHERE id='$otp_id'");
	}
	...
	public static function SetOTPKey($user, $key)
	{
		...
		$vfy_exists = mysql_query("SELECT id FROM ramtable WHERE id='$otp_id'");
		$vfy_count = mysql_num_rows($vfy_exists);
		if($vfy_count != 1)
		{
			mysql_query("INSERT INTO ramtable (data) VALUES('$key')");
			$otp_id = mysql_insert_id();
		}
		else
		{
			mysql_query("UPDATE ramtable SET data='$key' WHERE (id='$otp_id')");
		}
		mysql_query("UPDATE accounts SET otp='$otp_id' WHERE (username='$user')");
		echo mysql_error();
	}
	...
	public static function SetUserCookie($username, $name, $data, $key = NULL)
	{
		...
		security::setbrowsercookie(security::ServerHash($name), $data);
	}
